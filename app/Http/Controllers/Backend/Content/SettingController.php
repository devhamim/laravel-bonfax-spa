...
//   }
  ...
  public function servicestore(Request $request)
  {
    ...
    //   $slider->ban_title = $request->ban_title ?? null;
      ...
      $slider->price = $request->price ?? null;
...
function secrets_store(Request $request){
    $request->validate([
        'title'=>'required',
        'video'=>'required',
        'secrets_des'=>'required',
        'secrets_img'=>'required',
        'secrets_img_l'=>'required',
        'secrets_img_r'=>'required',
    ]);

    // secrets_img
    $req_img = $request->secrets_img;
    $extention = $req_img->getClientOriginalExtension();
    $file_name = rand(100000,999999).'.'.$extention;
    Image::make($req_img)->save(public_path('backend_img/secrets/'.$file_name));
...
function admin_setting_secrets_update(Request $request){
    ...
    //     'video'=>$request->video,
    //     'secrets_des'=>$request->secrets_des,
    //     'secrets_img'=>$file_name ?? $request->secrets_img,
    //     'secrets_img_l'=>$file_name_l ?? $request->secrets_img_l,
    //     'secrets_img_r'=>$file_name_r ?? $request->secrets_img_r,
    //     'is_active'=>$request->is_active,
    // ]);

    $secrets_update = secrets::find($secrets_id);
    $secrets_update->title = $request->title ?? $secrets_update->title;
    $secrets_update->video = $request->video ?? $secrets_update->video;
    $secrets_update->secrets_des = $request->secrets_des ?? $secrets_update->secrets_des;

    if($request->secrets_img != null){
        $req_img = $request->secrets_img;
        $extention = $req_img->getClientOriginalExtension();
        $file_name = rand(100000,999999).'.'.$extention;
        Image::make($req_img)->save(public_path('backend_img/secrets/'.$file_name));
        $secrets_update->secrets_img = $file_name ?? $secrets_update->secrets_img;
    }

    if($request->secrets_img_l != null){
        ...
        $secrets_update->secrets_img_l = $file_name_l ?? $secrets_update->secrets_img_l;
    }

    if($request->secrets_img_r != null){
        ...
        $secrets_update->secrets_img_r = $file_name_r ?? $secrets_update->secrets_img_r;
    }

    $secrets_update->is_active = $request->is_active ?? $secrets_update->is_active;
    $secrets_update->save();



    return redirect()->route('admin.setting.secrets')->withFlashSuccess('About Update Successfully');
...
function video_store(Request $request){
    ...
    // video banner image
    $ban_imgs = $request->ban_img;
    $extention = $ban_imgs->getClientOriginalExtension();
    $file_name = rand(100000,999999).'.'.$extention;
    Image::make($ban_imgs)->save(public_path('backend_img/videos/'.$file_name));


    // video thumbnail
    $thum_imgs = $request->thumbnail;
    $extentions = $thum_imgs->getClientOriginalExtension();
    $thum_name = rand(100000,999999).'.'.$extentions;
    Image::make($thum_imgs)->save(public_path('backend_img/videos/'. $thum_name));
...
// video_update
    function video_update(Request $request){
        ...
        $video_up = videos::find($videos_id);
        $video_up->video_title = $request->video_title ?? $video_up->video_title;
        ...
        if($request->ban_img != null){
            ...
            $file_name = rand(100000,999999).'.'.$extention;
            Image::make($ban_imgs)->save(public_path('backend_img/videos/'. $file_name));
            $video_up->ban_img = $file_name ?? $video_up->ban_img;
        }
        if($request->thumbnail != null){
            // video thumbnail image
            $thum_imgs = $request->thumbnail;
            $extention = $thum_imgs->getClientOriginalExtension();
            $thum_name = rand(100000,999999).'.'.$extention;
            Image::make($thum_imgs)->save(public_path('backend_img/videos/'. $thum_name));
            $video_up->thumbnail = $thum_name ?? $video_up->thumbnail;
        }
        $video_up->video_url = $request->video_url ?? $video_up->video_url;
        $video_up->save();
